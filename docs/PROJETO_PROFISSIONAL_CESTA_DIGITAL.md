...
Próximo gate: atualização isolada da dependência `cryptography` e homologação
local para validação UX manual. O `NO-GO` para dados reais e publicação externa
permanece até a conclusão dos gates profissionais.
