...
| 19/08/2026 | Checkpoint local V2-06 — Famílias | Branch `feat/frontend-v2-familias` migra lista e detalhe para a composição da referência 02: tabela e painel selecionado no desktop, cards em 360/390/768 e filtros, paginação e seleção no URL. Nome familiar e CPF não foram inventados; um único detalhe é carregado sob demanda. O detalhe separa sugestão calculada, decisão registrada e prioridade social, apresentando aptidão antes do resumo no mobile. Rotas, RBAC, APIs e backend não mudaram. Gates: lint/build, 35/35 E2E, 13 testes visuais próprios aprovados com 3 skips desktop-específicos e 11 testes do shell aprovados com 1 skip desktop intencional. A galeria aguarda aprovação. |

Próximo marco do frontend, condicionado à aprovação visual do V2-06: `V2-07A`,
fechar o contrato global e a integridade da regra de Avaliações antes do V2-07B
visual. A continuidade da Fase 2 de domínio permanece em backlog separado e não
deve ser misturada aos PRs visuais.
