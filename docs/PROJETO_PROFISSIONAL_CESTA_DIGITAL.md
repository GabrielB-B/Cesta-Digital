...
| 26/08/2026 | Checkpoint local V2-11 — Tipos de Cesta proposto | Branch `feat/frontend-v2-tipos-cesta` reproduz a referência 07 com cartões de tipos, composição selecionada e resumo no desktop, além de cartões próprios em 360/390/768. `GET /basket-types/overview` calcula quantidade de produtos e valor de referência no servidor; a edição preserva a disponibilidade baseada em estoque utilizável. Imagens vêm do catálogo persistido e duplicação inexistente não foi simulada. Escritas e RBAC `admin`/`operador` foram preservados, sem migration. Gates: backend 71/71, frontend lint/build, E2E 37/37 e visual 8 aprovados com 4 skips intencionais. Aguarda aprovação visual de Gabriel. |

Próximo gate: aprovação visual da aba Tipos de Cesta (`V2-11`) na galeria
comparativa em 390/768/1440. Relatórios (`V2-12`) não avança antes dessa
decisão.
